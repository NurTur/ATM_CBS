...
VALUES ( @ContractId, @FileRowId )", entity, UnitOfWork.Transaction);
        ...
        public void Delete(int id)
        {
            using (var transaction = BeginTransaction())
            {
                UnitOfWork.Session.Execute(@"DELETE FROM ContractFileRows WHERE Id = @id", new { id }, UnitOfWork.Transaction);
                transaction.Commit();
            }
        }
