...
namespace Pawnshop.Web.Engine.Middleware
{
    ...
    public class EventAttribute : Attribute, IActionFilter
    {
        ...
        {
            _reqData = null;
            if (EventMode == EventMode.Request)
            {
        ...
        public void OnActionExecuted(ActionExecutedContext context)
        {
            ...
            {
                Log(context.HttpContext, Code, EventStatus.Failed, EntityType, _entityId, _reqData, context.Exception.Message);
                return;
            }

            string resData = null;
            if (EventMode == EventMode.Response && context.Result is ObjectResult)
            {
        ...
        private void Log(HttpContext context, EventCode code, EventStatus status, EntityType entityType, int? entityId, string requestData, string responseData)
        {
            ...
            var eventLog = scope.Resolve<EventLog>();
